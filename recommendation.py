...
def F(partial_sums, new):
   ...
   #Diversity term
   C = (partial_sums[CRITERIA[1:]] + new[CRITERIA[1:]]).apply(lambda x: x**(alpha)).sum() 
   return R + l*C
...
def deterministic_greedy(data, n_vid=10, q = 0.15, l = 1/10, alpha = 0.5):
    ...
    for i in range(n_complete):
        ...
        obj = df.loc[~df['uid'].isin(S1)].apply(lambda x: F(partial_sums, x), axis='columns')

        #Update S1 and partial sums
        new = df.loc[obj.idxmax(), 'uid']
        S1.append(new)
        partial_sums = (partial_sums + df.loc[df['uid']==new, CRITERIA]).iloc[0] #hack to keep a series
    ...
    partial_sum = 0
    df_incomplete = df.loc[contains_na,['uid', 'largely_recommended']]    #we remove videos with no missing score
    df_incomplete = df_incomplete[~df_incomplete['uid'].isin(S1)] #we remove videos that were already selected

    for i in range(n_incomplete):
        #Compute the objective function
        obj = df_incomplete.loc[~df_incomplete['uid'].isin(S2)].apply(lambda x: R(partial_sum, x), axis='columns')

        #Update S2 and partial sums
        new = df_incomplete.loc[obj.idxmax(), 'uid']
        S2.append(new)
        partial_sum = (partial_sum + df_incomplete.loc[df_incomplete['uid']==new, 'largely_recommended']).iloc[0] #hack to get a series 
...
def random(data, n_vid = 10, pre_selection = False, threshold=0, alpha = 0.5, l = 1/10):
    ...
    obj = sums['largely_recommended'] + l*sums[CRITERIA[1:]].apply(lambda x: x**alpha).sum()
...
if __name__ == '__main__': 
    ...
    if len(sys.argv) == 1:    #no data file provided 
        ...
        dataFrame.to_csv('tournesol_scores_' + str(datetime.datetime.today()).split(' ')[0] + '.csv')
    else:
    ...
    if len(sys.argv) < 3:    #no results file provided
        ...
        for k in range(n_tests):
            ...
            df = dataFrame.loc[np.random.choice(a=dataFrame.index, size=size, replace=False)]
            ...
            r_thresh_0 = random(df, n_vid=n_vid, alpha=alpha, pre_selection=True, threshold=0)
            ...
            r_thresh_20 = random(df, n_vid=n_vid, alpha=alpha, pre_selection=True, threshold=20)
        ...
        results.to_csv('tests_' + 'n_test=' + str(n_tests) + '_size=' + str(size) + '.csv')
        
    #### PLOTS ####
    
    def unique_channel(df, uids):    #used to count how many channel are featured in each selection
    ...
    for alg in compare_with:
        ...
        for i in range(len(X)):
            sns.histplot(
                    data=results.loc[results['algorithm']=='deterministic_greedy', X] - results.loc[results['algorithm']==alg, X],
                    x=X[i], 
                    element='step', 
                    ax=axs[i%3, i%4], 
                    legend=(i==0)
        ...
        sns.histplot(data=results[results['algorithm'].isin(['deterministic_greedy', alg])], x='n_channel', hue='algorithm', element='step', ax=axs[2, 3])
    ...
    for alg in compare_with:
        ...
        for i in range(len(X)):
            sns.histplot(
                    data=results.loc[results['algorithm']=='random_greedy', X] - results.loc[results['algorithm']==alg, X],
                    x=X[i], 
                    element='step', 
                    ax=axs[i%3, i%4], 
                    legend=(i==0)
        ...
        n_vid_per_recommendation = len(results.loc[1, 'uids'])
        results['n_channel'] = results['uids'].apply(lambda x: unique_channel(dataFrame, x))
        sns.histplot(data=results[results['algorithm'].isin(['random_greedy', alg])], x='n_channel', hue='algorithm', element='step', ax=axs[2, 3])
